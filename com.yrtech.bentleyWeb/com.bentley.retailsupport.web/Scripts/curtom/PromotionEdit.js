...
function BMCSendEmail() {
    ...
    if (!(roleName == "SYSADMIN" || roleName == "BMC")) {
        MarketActionBefore21.KeyVisionApprovalCode = VisualStatusOld;
        MarketActionBefore21.KeyVisionApprovalDesc = VisualDescriptionOld;
    }
    var data = {
        MarketActionId: MarketActionBefore21.MarketActionId,
        TarketModelCode: $("#MarketActionTargetModelCode").val(),
        MarketActionBefore4Weeks: MarketActionBefore21
    };
    if (bmcSendEmail == 0) {
        bmcSendEmail = 1;
        ...
        if (roleName != "BMC") {
            if ($("#ossImage-100").attr("data-flag") == 3) {
                $.commonPost("MarketAction/MarketActionBefore4WeeksSave", {
                    UserId: $("#G_UserId").val(),
        ...
        if (((VisualDescriptionOld != VisualDescription) || (VisualStatus != VisualStatusOld))) //有邮件地址并且内容被修改了
        {
            $.commonPost("MarketAction/MarketActionBefore4WeeksSave", {
                ...
                $.commonGet("MarketAction/KeyVisionSendEmailToShop", { marketActionId: id }, function (data) {
                    var msg = isZH() ? '提交成功！' : "Save successfully !";
                    layer.alert(msg, {
                        skin: 'layui-layer-molv', closeBtn: 0
                    }, function (datas) {
